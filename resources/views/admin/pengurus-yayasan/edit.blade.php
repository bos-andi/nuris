...
@section('title', 'Edit Pengurus Yayasan')

@section('content')
<div class="mb-6">
    <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Edit Pengurus Yayasan</h4>
    <p class="text-sm text-gray-500 dark:text-gray-400">Edit data pengurus yayasan</p>
</div>

<div class="card">
    <div class="p-6">
        <form action="{{ route('admin.pengurus-yayasan.update', $pengurus->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" id="jabatan" name="jabatan" class="form-input w-full" value="{{ old('jabatan', $pengurus->jabatan) }}" placeholder="Contoh: Ketua, Sekretaris, Bendahara" required>
                @error('jabatan')
                    <p class="text-danger text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="jabatan_lengkap" name="jabatan_lengkap" class="form-input w-full" value="{{ old('jabatan_lengkap', $pengurus->jabatan_lengkap) }}" placeholder="Contoh: Ketua Yayasan PP. Nurul Islam">
            ...
            <div class="mb-4">
                ...
                <select id="kategori" name="kategori" class="form-select w-full">
                    ...
                    <option value="Utama" {{ old('kategori', $pengurus->kategori) == 'Utama' ? 'selected' : '' }}>Utama</option>
                    <option value="Staf Bidang Administrasi" {{ old('kategori', $pengurus->kategori) == 'Staf Bidang Administrasi' ? 'selected' : '' }}>Staf Bidang Administrasi</option>
                    <option value="Staf Bidang Keuangan" {{ old('kategori', $pengurus->kategori) == 'Staf Bidang Keuangan' ? 'selected' : '' }}>Staf Bidang Keuangan</option>
                </select>
